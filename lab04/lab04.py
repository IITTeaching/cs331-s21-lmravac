...
class ArrayList:
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        if not (isinstance(idx, int) and idx <= self.len):
            raise IndexError
    ...
    def pop(self, idx=-1):
        ...
        by default)."""
        ### BEGIN SOLUTION
        #set val to some temp var, set to none, move everything in front back 1, return temp
        assert(isinstance(idx, int) and idx < self.len)
        holder = self.data[idx]
        if idx == -1:
            self.len -= 1
            return holder
    ...
    def remove(self, value):
        ...
        ### BEGIN SOLUTION
        #iterate through whole list, return if equal, if never returned raise value error.
    ...
    def __eq__(self, other):
        ...
        ### BEGIN SOLUTION
        if self.len != len(other) or (not(isinstance(other, ArrayList))):
            return False
    ...
    def __add__(self, other):
        ...
        of other."""
        ### BEGIN SOLUTION
        temp = ArrayList(self.len + len(other))
        for i in range(self.len):
            temp[i] = self.data[i]
        for i in range(len(other)):
            temp[i + self.len] = other[i]
        return temp
        ### END SOLUTION
